...
use crate::{
    ...
    components::{
      file::{filter::component::FileFilterComponent, priority::component::FilePriorityComponent}, dir::{filter::component::DirFilterComponent, priority::{dir::component::DirPriorityComponent, file::component::DirFilePriorityComponent}},
    },
};
...
impl FileListComponent {
    pub fn ui<B: Backend>(app: &mut App, f: &mut Frame<B>, chunks: &Vec<Rect>) {
        ...
        let chunks = FileListComponent::get_main_chunks(file_list.state.is_selected(), chunks);
        ...
        match entry.is_dir() {
            true =>
                match app.components.file_list.state.is_priority_mode {
                    true => {
                        DirFilePriorityComponent::ui(app, f, &action_chunks);
                        DirPriorityComponent::ui(app, f, &action_chunks);
                    }
                    false => {
                        FileFilterComponent::ui(app, f, &action_chunks);
                        DirFilterComponent::ui(app, f, &action_chunks);
                    }
                }
            false => {
