...
use crate::application::{ app::{ App, ACTIVE_BORDER_COLOR }, mode::AppMode };

use super::component::TabComponent;
...
impl TabComponent {
    pub fn ui<B: Backend>(app: &mut App, f: &mut Frame<B>, chunks: &Vec<Rect>) {
        ...
        let headers = TabComponent::create_headers(&tabs.state.headers, tabs.state.index);
        let tabs = TabComponent::create_tabs(headers, app.state.mode == AppMode::Tab);
